    ...
    <div class="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow">
            ...
            <div class="flex flex-col md:flex-row gap-4 p-6 border-b">
                ...
                <div class="md:w-2/3">
                    ...
                    <p class="text-gray-500 text-sm mt-2">
                        <span class="font-medium">Kapasitas:</span> {{ $lapangan->kapasitas }} orang
                    </p>
                </div>
            </div>
        </div>
        ...
        <form class="space-y-6" action="{{ route('pemesanan.store') }}" method="POST">
            ...
            <div>
                ...
                <input type="text" 
                       value="{{ $lapangan->nama_lapangan }} - {{ $lapangan->tipe_lapangan }} - Rp {{ number_format($lapangan->harga_per_jam, 0, ',', '.') }}/jam"
            ...
            <div>
                ...
                <select name="duration" id="duration" class="w-full mt-2 border rounded-lg px-4 py-2" required>
                    <option value="1">1 Jam - Rp {{ number_format($lapangan->harga_per_jam * 1, 0, ',', '.') }}</option>
                    <option value="2">2 Jam - Rp {{ number_format($lapangan->harga_per_jam * 2, 0, ',', '.') }}</option>
                    <option value="3">3 Jam - Rp {{ number_format($lapangan->harga_per_jam * 3, 0, ',', '.') }}</option>
                </select>
            </div>
            ...
            {{-- TAMBAHAN 2: RINGKASAN HARGA --}}
            <div class="bg-gray-50 p-4 rounded-lg border">
                <h3 class="font-semibold mb-2">Ringkasan Harga</h3>
                <div class="flex justify-between text-gray-600 text-sm">
                    <span>{{ $lapangan->nama_lapangan }}</span>
                    <span>Rp {{ number_format($lapangan->harga_per_jam, 0, ',', '.') }} x <span id="duration-display">1</span> jam</span>
                </div>
